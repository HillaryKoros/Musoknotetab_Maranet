...
const MONITORING_STATIONS_CONFIG = {
  style: { radius: 8, fillColor: "#3388ff", color: "#fff", weight: 1, opacity: 1, fillOpacity: 0.8, selectedFillColor: "#ff4444" }
};
...
const getFloodHazardDate = () => new Date().toISOString().slice(0, 10).replace(/-/g, '');
...
const BASE_MAPS = [
  { name: 'ICPAC', url: 'https://eahazardswatch.icpac.net/tileserver-gl/styles/droughtwatch/{z}/{x}/{y}.png', attribution: '© ICPAC_FloodWatch' },
  { name: 'OpenStreetMap', url: 'https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', attribution: '© OpenStreetMap contributors' },
  { name: 'Satellite', url: 'https://server.arcgisonline.com/ArcGIS/rest/services/World_Imagery/MapServer/tile/{z}/{y}/{x}', attribution: '© ESRI, Maxar' },
  { name: 'Topographic', url: 'https://{s}.tile.opentopomap.org/{z}/{x}/{y}.png', attribution: '© OpenTopoMap contributors' },
];
      ...
      {layers.map((layer) => (
        <ListGroup.Item key={layer.name}>
          <div className="toggle-switch-small">
            <input type="checkbox" id={`layer-${layer.name}`} checked={selectedLayers.has(layer.layer)} onChange={() => onLayerSelect(layer)} />
    ...
    <div className="chart-container">
      <ResponsiveContainer>
        <LineChart data={processedData} margin={{ top: 20, right: 30, left: 70, bottom: 80 }}>
          ...
          <YAxis label={{ value: 'Discharge (m³/s)', angle: -90, position: 'insideLeft', offset: -50, style: { fontSize: '12px', fontWeight: 'bold' } }} />
    ...
    <div className="chart-container">
      <ResponsiveContainer>
        <LineChart data={timeSeriesData} margin={{ top: 20, right: 30, left: 70, bottom: 80 }}>
          ...
          <YAxis label={{ value: yAxisLabel, angle: -90, position: 'insideLeft', offset: -50, style: { fontSize: '12px', fontWeight: 'bold' } }} tickFormatter={(value) => Number(value).toFixed(2)} />
...
const MapLegend = ({ legendUrl, title }) => {
  const needsCustomLegend = () => title === 'Alerts Map' || legendUrl?.includes('floodwatch:Alerts') || title === 'GeoFSM' || legendUrl?.includes('floodwatch:geofsm_layer');
  const legendData = needsCustomLegend() ? (title === 'GeoFSM' || legendUrl?.includes('floodwatch:geofsm_layer') ? { title: 'GeoFSM', items: [{ color: '#2c7fb8', label: 'Low Risk' }, { color: '#7fcdbb', label: 'Medium Risk' }, { color: '#edf8b1', label: 'High Risk' }] } : { title: 'Alerts Map', items: [{ color: '#FF0000', label: 'Group 4 Alert' }, { color: '#FFFF00', label: 'Group 2 Alert' }, { color: '#45cbf7', label: 'Group 1 Alert' }] }) : null;
  ...
  return needsCustomLegend() ? (
    <div className="map-legend">
      ...
      {legendData.items.map((item, index) => (
        <div key={index} style={{ display: 'flex', alignItems: 'center', marginBottom: '8px' }}>
          <div style={{ width: '24px', height: '24px', backgroundColor: item.color, marginRight: '8px', border: '1px solid #ccc' }} />
    ...
    <div className="map-legend">
      <h5>{title}</h5>
      <img src={legendUrl} alt={`Legend for ${title}`} onError={(e) => e.target.style.display = 'none'} />
    </div>
  ) : null;
};

const TabSidebar = ({ hazardLayers, impactLayers, selectedLayers, onLayerSelect, showMonitoringStations, setShowMonitoringStations, showGeoFSM, setShowGeoFSM, selectedStation }) => (
  <div className="sidebar">
    <Tab.Container defaultActiveKey="forecast">
      <Nav variant="tabs" className="sidebar-tabs">
        <Nav.Item><Nav.Link eventKey="forecast" className="tab-link">Sector Layers</Nav.Link></Nav.Item>
        <Nav.Item><Nav.Link eventKey="monitoring" className="tab-link">Impact Layers</Nav.Link></Nav.Item>
      </Nav>
      <Tab.Content>
        <Tab.Pane eventKey="forecast" className="tab-pane">
          ...
          <ListGroup className="mb-4">
            <ListGroup.Item>
              <div className="toggle-switch-small">
                <input type="checkbox" id="monitoring-stations-toggle" checked={showMonitoringStations} onChange={() => setShowMonitoringStations(prev => !prev)} />
            ...
            <ListGroup.Item>
              <div className="toggle-switch-small">
                <input type="checkbox" id="geofsm-toggle" checked={showGeoFSM} onChange={() => setShowGeoFSM(prev => !prev)} />
          ...
          {selectedStation && (
            <div className="station-characteristics">
              ...
              <div className="characteristics-grid">
                <div className="characteristic-item"><span className="characteristic-label">Basin:</span><span className="characteristic-value">{selectedStation.properties?.BASIN}</span></div>
                <div className="characteristic-item"><span className="characteristic-label">Area:</span><span className="characteristic-value">{selectedStation.properties?.AREA} km²</span></div>
                <div className="characteristic-item"><span className="characteristic-label">Location:</span><span className="characteristic-value">{selectedStation.properties?.latitude?.toFixed(4)}°N, {selectedStation.properties?.longitude?.toFixed(4)}°E</span></div>
                <div className="characteristic-item"><span className="characteristic-label">Alert Threshold:</span><span className="characteristic-value alert-threshold">{selectedStation.properties?.Q_THR1} m³/s</span></div>
                <div className="characteristic-item"><span className="characteristic-label">Alarm Threshold:</span><span className="characteristic-value alarm-threshold">{selectedStation.properties?.Q_THR2} m³/s</span></div>
                <div className="characteristic-item"><span className="characteristic-label">Emergency Threshold:</span><span className="characteristic-value emergency-threshold">{selectedStation.properties?.Q_THR3} m³/s</span></div>
              </div>
            </div>
          )}
        </Tab.Pane>
        <Tab.Pane eventKey="monitoring" className="tab-pane">
          <LayerSelector title="Inundation Map" layers={hazardLayers} selectedLayers={selectedLayers} onLayerSelect={onLayerSelect} />
          <LayerSelector title="Impact Layers" layers={impactLayers} selectedLayers={selectedLayers} onLayerSelect={onLayerSelect} />
        </Tab.Pane>
      </Tab.Content>
    </Tab.Container>
  </div>
  ...
  useMapEvents({
    click: async (e) => {
      ...
      const params = new URLSearchParams({
        SERVICE: 'WMS', VERSION: '1.1.1', REQUEST: 'GetFeatureInfo', QUERY_LAYERS: layersArray.join(','), LAYERS: layersArray.join(','),
        INFO_FORMAT: MAP_CONFIG.getFeatureInfoFormat, X: Math.round(point.x), Y: Math.round(point.y), WIDTH: size.x, HEIGHT: size.y,
        BBOX: `${bounds.getWest()},${bounds.getSouth()},${bounds.getEast()},${bounds.getNorth()}`, SRS: 'EPSG:4326', FEATURE_COUNT: 10
      });
      ...
      try {
        ...
        if (data?.features?.length > 0) {
          onFeatureInfo(data.features.map(feature => ({ feature, position: e.latlng })));
        }
      } catch (error) {
...
const MapViewer = () => {
  ...
  const [selectedBoundaryLayers, setSelectedBoundaryLayers] = useState(new Set([BOUNDARY_LAYERS[0].layer])); 
    ...
    if (showGeoFSM) {
      fetch('hydro_data_with_locations.geojson')
        ...
        .then(data => {
          ...
          const dataTypes = [...new Set(data.features.map(f => f.properties.data_type).filter(type => type && validTypes.includes(type)))];
  ...
  const handleLayerSelection = useCallback((layer) => {
    setSelectedLayers(prev => {
      ...
      if (newSelectedLayers.has(layer.layer)) {
        ...
        if (isImpactLayer) IMPACT_LAYERS.forEach(l => newSelectedLayers.delete(l.layer));
  ...
  const handleBoundaryLayerSelection = useCallback((layer) => {
    ...
    <div className="map-viewer">
      ...
      <div className="main-content">
        <div className="map-container">
          <MapContainer center={MAP_CONFIG.initialPosition} zoom={MAP_CONFIG.initialZoom} scrollWheelZoom={true} style={{ height: '100%', width: '100%' }} whenCreated={setMap} key={mapKey}>
            <TileLayer url={BASE_MAPS[0].url} attribution={BASE_MAPS[0].attribution} />
            <div className="toggle-label" onClick={() => setIsLayerControlVisible(!isLayerControlVisible)}>Drought Watch</div>
            <div className={`layer-control-panel ${isLayerControlVisible ? 'visible' : ''}`}>
              <button className="layer-control-close-btn" onClick={() => setIsLayerControlVisible(false)}>Close</button>
              <LayersControl position="topright">
                ...
                {BOUNDARY_LAYERS.map((layer) => (
                  <LayersControl.Overlay key={layer.layer} name={layer.name} checked={selectedBoundaryLayers.has(layer.layer)}>
                  <WMSTileLayer url={MAP_CONFIG.geoserverWMSUrl} layers={layer.layer} format="image/png" transparent={true} version="1.1.1" eventHandlers={{ add: () => handleBoundaryLayerSelection(layer), remove: () => handleBoundaryLayerSelection(layer) }} />
                </LayersControl.Overlay>
                ))}
                {Array.from(selectedLayers).map((layer) => (
                  <LayersControl.Overlay key={`${layer}-${mapKey}`} checked={true} name={layer.split(':')[1] || 'Layer'}>
                    <WMSTileLayer url={MAP_CONFIG.geoserverWMSUrl} layers={layer} format="image/png" transparent={true} version="1.1.1" />
                  </LayersControl.Overlay>
                ))}
              </LayersControl>
            </div>
            {showMonitoringStations && monitoringData?.features && (
              <GeoJSON
                ...
                pointToLayer={(feature, latlng) => {
                  ...
                  return L.circleMarker(latlng, { ...MONITORING_STATIONS_CONFIG.style, fillColor: isSelected ? MONITORING_STATIONS_CONFIG.style.selectedFillColor : MONITORING_STATIONS_CONFIG.style.fillColor });
                }}
                onEachFeature={(feature, layer) => {
                  layer.on({ click: () => handleStationClick(feature) });
                  const props = feature.properties;
                  layer.bindPopup(`<div class="station-popup"><strong>${props.SEC_NAME || 'Station'}</strong><br/>Basin: ${props.BASIN || 'N/A'}<br/>Current Status: ${props.status || 'Normal'}</div>`);
                }}
              />
            )}
            {showGeoFSM && geoFSMData?.features && (
              <GeoJSON
                ...
                data={geoFSMData}
                pointToLayer={(feature, latlng) => L.circleMarker(latlng, { radius: 6, fillColor: "#2c7fb8", color: "#fff", weight: 1, opacity: 1, fillOpacity: 0.8 })}
                onEachFeature={(feature, layer) => {
                  const props = feature.properties;
                  layer.bindPopup(`<div class="geofsm-popup"><strong>${props.Name || 'GeoFSM Point'}</strong><br/>Description: ${props.Descriptio || 'N/A'}<br/>Gridcode: ${props.Gridcode || 'N/A'}<br/>Latitude: ${props.Y?.toFixed(4) || 'N/A'}°N<br/>Longitude: ${props.X?.toFixed(4) || 'N/A'}°E<br/>ID: ${props.Id || 'N/A'}</div>`);
                  layer.on({ click: () => handleStationClick(feature) });
                }}
              />
            )}
            <FeatureInfoHandler map={map} selectedLayers={new Set([...selectedLayers, ...selectedBoundaryLayers])} onFeatureInfo={handleFeatureInfo} />
            {featurePopups.map((popup, index) => (
              <Popup key={`popup-${index}-${popup.position.lat}-${popup.position.lng}`} position={[popup.position.lat, popup.position.lng]} onClose={() => setFeaturePopups(current => current.filter((_, i) => i !== index))}>
                <div className="feature-info">
                  {popup.feature.properties && Object.entries(popup.feature.properties).filter(([key]) => !key.startsWith('_') && key !== 'bbox').map(([key, value]) => (
                    <div key={key} className="popup-row"><strong>{key}:</strong> {value}</div>
                  ))}
                </div>
              </Popup>
            ))}
          ...
          {activeLegend && (
            <MapLegend legendUrl={activeLegend} title={[...HAZARD_LAYERS, ...IMPACT_LAYERS, ...BOUNDARY_LAYERS].find(layer => layer.legend === activeLegend)?.name || 'Legend'} />
          )}
        </div>
        {showChart && (
          <div className="bottom-panel">
            <div className="chart-header">
              <h5>{selectedStation?.properties?.SEC_NAME || (chartType === 'riverdepth' ? 'GeoSFM River Depth' : chartType === 'streamflow' ? 'GeoSFM Streamflow' : 'Discharge Forecast')}</h5>
              {(chartType === 'riverdepth' || chartType === 'streamflow') && availableDataTypes.length > 1 && (
                <select value={geoFSMDataType} onChange={(e) => { setGeoFSMDataType(e.target.value); setChartType(e.target.value); }} style={{ marginLeft: '10px' }}>
                  {availableDataTypes.map(type => (
                    <option key={type} value={type}>{type === 'riverdepth' ? 'River Depth' : 'Streamflow'}</option>
                  ))}
                </select>
              )}
              <button className="close-button" onClick={() => { setShowChart(false); setSelectedStation(null); setTimeSeriesData([]); setGeoFSMTimeSeriesData([]); }}>×</button>
            </div>
            {chartType === 'riverdepth' || chartType === 'streamflow' ? (
              <GeoSFMChart timeSeriesData={geoFSMTimeSeriesData} dataType={geoFSMDataType} />
            ) : (
